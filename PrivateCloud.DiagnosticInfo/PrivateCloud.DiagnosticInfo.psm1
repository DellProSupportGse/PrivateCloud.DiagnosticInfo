...
function Get-SddcDiagnosticInfo
{
    ...
    If (Get-Variable PSSenderInfo -ErrorAction SilentlyContinue) {Show-Error "This module is not supported using a remote powershell session. Please run locally"}
    ...
    if (test-path "C:\Observability\OEMDiagnostics") {
      ...
      $CmdsToLog += 'Invoke-Command -ComputerName _C_ {Echo Get-ActionplanInstanceToComplete;try {(Get-ActionPlanInstances | ? Status -ne "Completed" | Sort StartDateTime | Select -last 1).ProgressAsXml} catch {}}'#,
		    #'Invoke-Command -ComputerName _C_ {Get-ASEvent -Path (Get-ChildItem "C:\Observability\OEMDiagnostics\*.etl" | Select -Last 1).fullname}',
                ...
                While ((Test-Path $LocalFileMsInfo -ErrorAction SilentlyContinue) -and $msinfo.HasExited -ne $True -and (Get-Item $LocalFileMsInfo -ErrorAction SilentlyContinue).LastWriteTime -ge (Get-Date).AddMinutes(-5)) {Sleep -Milliseconds 100}
            }
        }
